...
def showMeDaMoney(session, base_url, all_da_stonks):
    market = Market(session, base_url)
    # market.quotes('VXX')
    for astonk in all_da_stonks:
        # astonk
        market.quotes(str(astonk))
...
def getStonkSymbols():
    ...
    all_stonks = data['Name'].tolist()
    # print(all_stonks)
    return all_stonks

if __name__ == "__main__":
    all_da_stonks = getStonkSymbols()
    session, base_url = oauth()
    showMeDaMoney(session, base_url, all_da_stonks)